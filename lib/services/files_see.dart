                    ...
                    try {
                      if (st == "PDF") {
                        ...
                        if (result != null) {
                          ...
                          if (fileSizeInBytes > 2 * 1024 * 1024) { // 2 MB in bytes
                            Send.message(context, 'File size exceeds 2MB limit. Please select a smaller file.', false);

                          } else {
                            Send.message(context, "Uploading your file", true);
                            final storageRef = lk.FirebaseStorage.instance.ref();
                            final mountainsRef = storageRef.child("Task_${widget.title}_${name.text}.pdf"); // Change filename if needed
                        ...
                        if (file != null) {
                          // Check file size (Uint8List size is directly in bytes)
                          if (file.lengthInBytes > 2 * 1024 * 1024) { // 2 MB in bytes
                            Send.message(context, 'File size exceeds 2MB limit. Please select a smaller file.', false);

                          } else {
                            Send.message(context, "Uploading your file", true);
                            String photoUrl = await StorageMethods().uploadImageToStorage('Company_Task', file, true);
                            setState(() {
                              link.text = photoUrl;
                            });
                            Send.message(context, "Picture Uploaded ", true);
                          }
                        }
                      } else {
                        ...
                        if (result != null) {
                          ...
                          if (fileSizeInBytes > 2 * 1024 * 1024) { // 2 MB in bytes
                            ...
                            Send.message(context, "Uploading your file", true);
                            final storageRef = lk.FirebaseStorage.instance.ref();
                            final mountainsRef = storageRef.child("Task_${widget.title}_${name.text}.${result.files.single.extension}");
                            await mountainsRef.putFile(file);
                            String downloadUrl = await mountainsRef.getDownloadURL();
                            setState(() {
                              link.text = downloadUrl;
